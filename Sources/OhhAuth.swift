...
open class OhhAuth
{
        ...
        consumerCredentials cc: Credentials, userCredentials uc: Credentials?) -> String
    {
        ...
        let signingKey: String = (uc == nil || uc!.secret == nil )
            ? rfc3986encode(cc.secret!)
            : [cc.secret!, uc?.secret! ?? ""].map(rfc3986encode).joined(separator: "&")
        ...
        if( parameter["oauth_callback"] != nil ) {
            oAuthParameters["oauth_callback"] = parameter["oauth_callback"]!
        }

        /// [RFC-5849 Section 3.5.1](https://tools.ietf.org/html/rfc5849#section-3.5.1)
        let signature = "OAuth " + oAuthParameters
            .map(tuplify)
            .sorted(by: cmp)
